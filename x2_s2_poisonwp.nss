...
void main()
{
  ...
  if (nRow ==0)
  {
   ...
   if (!bHasFeat) // without handle poison feat, do ability check
   {
       ...
       int d20roll = d20(1);
       int nCheck = d20roll + nDex;
       if (nCheck < nApplyDC)
       {
           if( d20roll == 1 || nCheck < (nApplyDC - 9)){
               // oPC poisons self nPoisonType
               int save = FortitudeSave(oPC, nApplyDC - 2, SAVING_THROW_TYPE_POISON);
               if(save == 0){
                   effect eDam = EffectAbilityDecrease(nPoisonType, d2(1));
                   effect eDur2 = EffectVisualEffect(VFX_DUR_CESSATE_NEGATIVE);
                   effect ePois = EffectLinkEffects(eDam, eDur2);
                   DelayCommand(0.1, ApplyEffectToObject(DURATION_TYPE_PERMANENT, ePois, oPC));
                   PlayVoiceChat(VOICE_CHAT_CUSS, oPC);
               }
           }
           FloatingTextStrRefOnCreature(83368,oPC);
           return;
           ////////////////////  end customize //////////
       }
       else
