...
void DoTrapSpike(int nDamage)
{
    ...
    SpeakString("try to spike trap   ", TALKVOLUME_SHOUT);
    ...
    int ispoisoned = GetLocalInt( OBJECT_SELF, "ispoisoned");
   // ispoisoned = TRUE;//for debugging
    SpeakString("try to poison 1   ", TALKVOLUME_SHOUT);
    if(ispoisoned == TRUE){
        SpeakString("try to poison 2   "   , TALKVOLUME_SHOUT);
        int poisonid = GetLocalInt( OBJECT_SELF, "poisonid");
       // poisonid = POISON_OIL_OF_TAGGIT;
        effect ePoison = EffectPoison(poisonid);
...
//    int nDamage = 0;
    ...
    if(nSpell == SPELL_MAGIC_MISSILE){
        nMissiles = nMissiles/2;
    }
    ...
    /**  for altereffects getAlteredMissileEffect call **/
    object component;
    object componentbag;
    // if has item in component pouch
    // only check first item in pouch
    // - player should  usually only have one item in there at a time

    effect eDam;
    float altFactor = 1.0;
    int alteredType = nDAMAGETYPE;
    if (nMetaMagic == METAMAGIC_SILENT &&
        (nSpell == SPELL_MAGIC_MISSILE ||
         nSpell == SPELL_ISAACS_LESSER_MISSILE_STORM ||
         nSpell == SPELL_ISAACS_GREATER_MISSILE_STORM) )
    {
        componentbag = GetItemPossessedBy(OBJECT_SELF, "craftingbag");
        if( GetIsObjectValid(componentbag)){
            component = GetFirstItemInInventory(componentbag);
            if(GetIsObjectValid(component)){
                altFactor = getAlteredMissileFactor( component, altFactor  );
                alteredType = getAlteredMissileType( component, nDAMAGETYPE  );
                if(alteredType == 0) nDAMAGETYPE = DAMAGE_TYPE_ACID;
            }
        }
    }
           ...
           (GetObjectSeen(oTarget,OBJECT_SELF))))
        {
                ...
                if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
                {
                    for (i=1; i <= nExtraMissiles + nRemainder; i++)
                    {
                        ...
                        if (nMetaMagic == METAMAGIC_SILENT)
                        {
                            nDam = FloatToInt( (nDam * altFactor));
                            if(nDam == 0 ){ nDam = 1; }
                            if(alteredType == 0 ){
                                 //each missile can be either acid or fire
                                 if(nDAMAGETYPE == DAMAGE_TYPE_ACID)  {
                                    nDAMAGETYPE = DAMAGE_TYPE_FIRE;
                                    // VFX_IMP_MIRV_FLAME, VFX_IMP_FLAME_M,
                                    eMissile = EffectVisualEffect(VFX_IMP_MIRV_FLAME);
                                    eVis = EffectVisualEffect(VFX_IMP_FLAME_M);
                                 }else{
                                    nDAMAGETYPE = DAMAGE_TYPE_ACID;
                                    eMissile = EffectVisualEffect(245 );
