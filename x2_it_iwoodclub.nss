...
void main()
{
    ...
    SpeakString("ironwood c ", TALKVOLUME_SHOUT);
    ...
    if (nEvent == X2_ITEM_EVENT_ACQUIRE)
    {
        ...
        if(GetLocalInt(item, "removedip") > 0){
            SpeakString("iwr already hardened!"  , TALKVOLUME_SHOUT);
        ...
        while (GetIsItemPropertyValid(ipLoop))
        {
            if (GetItemPropertyType(ipLoop) == ITEM_PROPERTY_ATTACK_BONUS) {
              SpeakString("Sorry, no AB to crafted weapons, but damage bonus instead!", TALKVOLUME_SHOUT);
