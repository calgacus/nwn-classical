...
*/
    ...
    int alteredType = DAMAGE_TYPE_ACID;
    int percentImm = 65;
    ...
    if (nMetaMagic == METAMAGIC_SILENT  )
    {
        ...
        if( GetIsObjectValid(componentbag)){
            ...
            if(GetIsObjectValid(component)){
                percentImm = percentImm + getAlteredSheathFactor( component );
                alteredType = getAlteredDamageShieldType( component, alteredType );
                if(percentImm > 100) percentImm = 100;
            }
        }
    }

    effect eImmune = EffectDamageImmunityIncrease(  alteredType, percentImm);

    //Link effects
    effect eLink = EffectLinkEffects(eImmune, eDur);
