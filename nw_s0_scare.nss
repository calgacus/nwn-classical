...
// End of Spell Cast Hook
   ...
   if ((GetHitDice(oTarget) < 6 || GetHitDice(oTarget) < GetCasterLevel(OBJECT_SELF) ) && GetObjectType(oTarget) == OBJECT_TYPE_CREATURE)
   {
        ...
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF) == TRUE)
        {
           ...
           if(!MyResistSpell(OBJECT_SELF, oTarget))
           {
                ...
                if(!/*Will Save*/ MySavingThrow(SAVING_THROW_WILL, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_FEAR))
                {
                   ...
                   ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink2, oTarget, RoundsToSeconds(nDuration+2));
                }
            }
        }
    }
