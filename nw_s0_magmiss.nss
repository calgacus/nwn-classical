...
*/
    ...
    DoMissileStorm(1, 6, SPELL_MAGIC_MISSILE, VFX_IMP_MIRV, VFX_IMP_MAGBLUE, DAMAGE_TYPE_MAGICAL, TRUE, FALSE );
     /*  doMissileStorm instead
        ...
        if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
        {
            ...
            for (nCnt = 1; nCnt <= nMissiles; nCnt++)
            {
                ...
                if (nMetaMagic == METAMAGIC_SILENT)
                {
                    ...
                    eDam = getAlteredMissileEffect( item, nDam, eDam );
                    DestroyObject(item);
                }else{
